...
NASDAQ_holtWinters <- function(filename){
  ...
  if(file.info(filename)[1]==0){
    ...
    if(fileLen < 755){
      ...
      #fitDataHW = HoltWinters(trainData, beta=FALSE, gamma=FALSE)
      ...
      {
        MAE[1,i] = abs(forecastDataHW$mean[i] - testData[i])
      }
...
output <- (lapply(fileList, NASDAQ_holtWinters))
...
finalHW <- head(final,10)
print("The top 10 stocks with the minimum sum of MAE using Holt-Winters Model")
print (finalHW)
length(finalHW$sumMAE)

# plot the top 10 minimum sum of MAE in 3 models respectively
plot(x=1:10, finalHW$sumMAE, col = "blue", main = "HoltWinters model", xlab = "Stock Index", ylab = "MAE sum")
lines(x=1:10, finalHW$sumMAE, lw = 2, col = "red")
